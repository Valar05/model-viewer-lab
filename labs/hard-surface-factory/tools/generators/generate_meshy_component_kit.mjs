...
const reviewCacheToken = process.env.MODEL_VIEWER_REVIEW_CACHE_TOKEN || reviewBranch.replace(/[^a-zA-Z0-9_.-]/g, '-');
function withReviewCacheToken(url) { return url + (url.includes('?') ? '&' : '?') + 'v=' + encodeURIComponent(reviewCacheToken); }
...
const components = [
  ...
  { id: 'tank_turret_housing', label: 'tank_turret_housing', sourceFilename: 'Meshy_AI_Tank_turret_housing_0717162705_texture.glb', filename: 'tank_turret_housing.glb', category: 'turret_reference_scaffold', intendedUse: 'source reference scaffold for turret housing proportions; rotated so mantlet/front detail shares the tank forward axis', participatesInDefaultAssembly: true, transform: { position: [0, 0.21, -0.04], rotationDeg: [0, 180, 0], scale: [0.38, 0.38, 0.38] } },
