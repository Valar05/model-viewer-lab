...
SERIES='real_sherman_chassis_modeled_platekit_scratch'; ASSET=SERIES+'_d05'; REV='d05-modeled-plates-more-structural-topology-no-box-assembly'
...
def upper(created,mats):
    ...
    for args in [(.06,.94,.09,cy-ro,22,8),(.07,.91,cy+ro,.91,22,8),(.06,cx-ro,cy-ro,cy+ro,8,14),(cx+ro,.94,cy-ro,cy+ro,8,14)]: patch(v,f,args[0],args[1],args[2],args[3],z,args[4],args[5])
    ...
    hv=[]; hf=[]; hx0,hx1,hy0,hy1=.33,.50,.22,.40; patch(hv,hf,hx0,hx1,hy0,hy1,lambda x,y:z(x,y)+.055,10,6)
...
def tray(created,mats):
    ...
    v=[]; f=[]; patch(v,f,.18,.82,.24,.76,lambda x,y:.34,12,7); obj(created,ASSET+'_01_recessed_floor_field',role,v,f,dark,'Inset floor field.',False,0)
...
def deck(created,mats):
    ...
    patch(v,f,.05,.95,.08,.92,z,28,14)
    for a,b in zip([(.05,.08),(.95,.08),(.95,.92),(.05,.92)],[ (.95,.08),(.95,.92),(.05,.92),(.05,.08)]): wall(v,f,a,b,z,.13)
    for x0 in [.22,.34,.46,.58,.70]: patch(v,f,x0,x0+.04,.22,.78,lambda x,y:.79,3,12)
...
def side(created,mats,role,tag,vertical=False):
    armor,dark,line=mats; v=[]; f=[]; stations=[.04,.09,.14,.20,.26,.33,.40,.48,.56,.64,.72,.80,.86,.92,.96]; sections=[]
    for s in stations:
...
def build():
    ...
    man={'asset_id':ASSET,'series_id':SERIES,'attempt':'d05','scratch_mode':True,'artifact_type':'scratch_modeled_plate_reference_kit','revision':REV,'source_manifest':str(SRC.relative_to(ROOT)),'reference_sheet_image':str(REF.relative_to(ROOT)),'configuration_policy':'Same seven source component roles and exploded/reference configuration as Meshy reference kit.','construction_rule':'Seven primary components are custom continuous meshes; rectangular feature-box assembly is intentionally not used.','budget':{'hard_cap_triangles':12000,'intent':'spend triangles on modeled plate transitions, apertures, and swept shells before optimizing'},'statistics_all_hand_mesh_excluding_reference_sheet':allstats,'statistics_primary_kit_pieces':pristats,'primary_kit_piece_count':len(primary),'all_hand_mesh_object_count':len(hand),'under_hard_cap':allstats['triangles']<=12000,'outputs':{'blend':str((blend/(ASSET+'.blend')).relative_to(ROOT)),'glb':str((model/(ASSET+'.glb')).relative_to(ROOT)),'front_with_reference_sheet':str((rend/'front_with_reference_sheet.png').relative_to(ROOT)),'hatch_deck_close':str((rend/'hatch_deck_close.png').relative_to(ROOT)),'rear_oblique':str((rend/'rear_oblique.png').relative_to(ROOT))},'parts':manifest_parts}
