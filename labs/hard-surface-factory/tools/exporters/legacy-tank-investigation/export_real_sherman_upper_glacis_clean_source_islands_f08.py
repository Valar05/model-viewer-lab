...
ASSET_ID = 'real_sherman_upper_glacis_clean_source_islands_scratch_f08'
REVISION = 'f08-source-faithful-clean-island-dissolve-reto'
...
def make_quadriflow_retopo(source_obj, material):
    # Start from the f05-winning premise: source-faithful topology, not analytic box/disk replacement.
    # Then remove the loose scan flakes that made f05 visually ragged.
    bpy.ops.object.select_all(action='DESELECT')
    source_obj.select_set(True)
    bpy.context.view_layer.objects.active = source_obj
    bpy.ops.object.duplicate()
    duplicate = bpy.context.object
    duplicate.name = ASSET_ID + '_source_duplicate_for_clean_islands'
    ...
    deleted = []
    for part in parts:
        tris = source_local_tri_count(part)
        b = source_bounds(part)
        sx = b['max'][0] - b['min'][0]
        sy = b['max'][1] - b['min'][1]
        sz = b['max'][2] - b['min'][2]
        areaish = sx * sy
        # Keep major armor/ring/hatch islands; drop tiny disconnected flakes that produce black slivers and seam trash.
        keep = tris >= 120 or (tris >= 45 and areaish >= 0.0012 and sz >= 0.006)
        reports.append({'source_name': part.name, 'source_triangles': tris, 'bounds': b, 'area_xy': areaish, 'keep': keep})
        if keep:
            kept.append(part)
        else:
            deleted.append(part)
    ...
    obj.name = ASSET_ID + '_upper_front_glacis_clean_source_islands'
    ...
    obj['primary_retopo_piece'] = True
    obj['authoring_policy'] = 'Source-faithful f05-style topology cleaned by deleting tiny loose source flakes, limited-dissolving kept islands, and preserving Sherman-specific ring/hatch/plate silhouette. This rejects f07 box-and-disk abstraction.'
    obj['clean_island_report_json'] = json.dumps(reports)
    obj.modifiers.new('weighted_normals_clean_source_islands', 'WEIGHTED_NORMAL')
    return obj
...
def main():
    ...
    landmarks = {
        ...
        'island_cleanup': 'delete tiny loose source flakes before dissolve; keep source-faithful silhouette islands',
        'retopo_engine': 'loose source island cleanup plus bpy.ops.mesh.dissolve_limited',
        'preserve_source_boundaries': True,
        'source_component': ROLE,
        'lesson_from_f01_f07': 'f05 was visually close but seamed; f07 became box-and-disk abstraction. f08 keeps f05 source identity and removes tiny loose islands that caused seam/sliver junk.'
    }
    ...
    note = '# ' + ASSET_ID + '\n\nSource-faithful clean-island hard-surface dissolve pass. Scratch diagnostic only; not cloud accepted.\n\n- Source component: ' + ROLE + '\n- Stats: ' + str(stats) + '\n- Depth report: ' + str(depth_report) + '\n- Intended to escape f07 abstraction while reducing f05 seam/sliver junk. This keeps source-faithful silhouette and feature ownership.\n'
