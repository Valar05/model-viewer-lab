...
ASSET_ID = 'real_sherman_upper_glacis_dissolve_retopo_scratch_f05'
REVISION = 'f05-source-topology-hard-surface-dissolve-reto'
...
def make_quadriflow_retopo(source_obj, material):
    ...
    obj.name = ASSET_ID + '_upper_front_glacis_dissolve_retopo'
    ...
    obj['primary_retopo_piece'] = True
    obj['authoring_policy'] = 'Source topology hard-surface limited dissolve at 35 degrees. This is a visual target pass: source faithful and non-ugly, but still over budget for final game use.'
    obj.modifiers.new('weighted_normals_dissolve_retopo', 'WEIGHTED_NORMAL')
    return obj
...
def main():
    ...
    landmarks = {
        ...
        'retopo_engine': 'bpy.ops.mesh.dissolve_limited over source topology',
        'preserve_source_boundaries': True,
        'source_component': ROLE,
        'lesson_from_f01_f02_f03_f04': 'f01 was a depth sheet; f02 guessed features; f03 bridged openings; f04 was ragged. f05 keeps source topology and dissolves hard-surface coplanar noise to establish a non-ugly visual target.'
    }
    ...
    note = '# ' + ASSET_ID + '\n\nSource-topology hard-surface dissolve retopo pass. Scratch diagnostic only; not cloud accepted.\n\n- Source component: ' + ROLE + '\n- Stats: ' + str(stats) + '\n- Depth report: ' + str(depth_report) + '\n- Intended to escape f01/f02/f03/f04 ugliness by keeping source topology and dissolving coplanar hard-surface noise. This is a visual target, not final game budget.\n'
