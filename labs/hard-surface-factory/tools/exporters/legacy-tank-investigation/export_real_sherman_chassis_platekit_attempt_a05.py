...
ATTEMPTS = [
    {'suffix': 'a05', 'label': 'wide-camera framed final candidate', 'spacing': 0.52, 'z_gap': 0.26, 'front_refine': 0.10, 'rear_refine': 0.08, 'socket_segments': 12, 'side_taper': 0.06, 'deck_cut': True, 'quad_side': True, 'ring_lip': False, 'judgment': 'Final scratch candidate: same clean quad-side topology as a04, with wide review cameras that frame the whole exploded kit.'},
]
...
def build_attempt(attempt):
    ...
    light.data.size = 5.5
    bpy.ops.object.camera_add(location=P(6.3, 4.4, 5.0))
    front_cam = bpy.context.object
    front_cam.name = asset_id + '_front_review_camera'
    bpy.ops.object.camera_add(location=P(-6.3, 4.25, 4.9))
    ...
    for cam in (front_cam, rear_cam):
        look_at(cam, Vector(P(0.0, 0.50, 0.0)))
        cam.data.lens = 30
