...
ASSET_ID = 'real_sherman_chassis_retopo_only_referencekit_scratch_e02'
REVISION = 'e02-source-vertex-band-retopo-only-no-envelope-authoring'
...
PROFILE = {
    'source_component_0_upper_front_glacis_assembly': {'rings': 10, 'bins': 48, 'axis': 2},
    'source_component_1_lower_hull_floor_tub': {'rings': 9, 'bins': 44, 'axis': 2},
    'source_component_2_engine_deck_assembly': {'rings': 8, 'bins': 42, 'axis': 2},
    'source_component_3_rear_plate_assembly': {'rings': 8, 'bins': 40, 'axis': 0},
    'source_component_4_left_side_assembly': {'rings': 8, 'bins': 40, 'axis': 1},
    'source_component_5_right_side_assembly': {'rings': 8, 'bins': 40, 'axis': 0},
    'source_component_6_remaining_reference_plate': {'rings': 7, 'bins': 36, 'axis': 1},
}
...
def angle_distance(a, b):
    d = abs((a - b + math.pi) % math.tau - math.pi)
    return d


def pick_band_sample(points, center, axis, axis_targets, ring_idx, bin_idx, bin_count):
    axes = [0, 1, 2]
    axes.remove(axis)
    a0, a1 = axes
    target_axis = axis_targets[ring_idx]
    target_angle = math.tau * (bin_idx / bin_count)
    best = None
    best_score = 10**18
    for src_i, p in points:
        ...
        if plen <= 1e-8:
            continue
        ang = math.atan2(planar.y, planar.x)
        axis_delta = abs(p[axis] - target_axis)
        angle_delta = angle_distance(ang, target_angle)
        # Favor coherent bands first, then the outer visible support in that angular sector.
        score = axis_delta * 8.0 + angle_delta * 0.55 - plen * 0.035
        if score < best_score:
            best_score = score
            best = (src_i, p)
    if best is None:
...
def build_shell_from_source(role, src_obj, material, provenance, created):
    ...
    bins = cfg['bins']
    axis = cfg['axis']
    axis_values = [p[axis] for _, p in points]
    axis_targets = [quantile(axis_values, r / max(1, rings - 1)) for r in range(rings)]
    verts = []
    src_indices = []
    for r in range(rings):
        for b in range(bins):
            src_i, p = pick_band_sample(points, center, axis, axis_targets, r, b, bins)
    ...
    if role == 'source_component_0_upper_front_glacis_assembly':
        ...
        for b in range(bins):
            src_i, p = pick_band_sample(points, center, axis, axis_targets, rings - 2, b, bins)
            verts.append(tuple(p))
            src_indices.append(src_i)
