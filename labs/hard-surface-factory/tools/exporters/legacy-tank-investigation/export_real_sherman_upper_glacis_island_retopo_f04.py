...
ASSET_ID = 'real_sherman_upper_glacis_island_retopo_scratch_f04'
REVISION = 'f04-loose-island-quadriflow-preserve-openings-reto'
...
def source_local_tri_count(obj):
...
def make_quadriflow_retopo(source_obj, material):
    ...
    duplicate.name = ASSET_ID + '_source_duplicate_for_loose_islands'
    ...
    for i, part in enumerate(parts):
        ...
        original_tris = source_local_tri_count(part)
        part.name = ASSET_ID + '_island_%02d_src_%dtris' % (i, original_tris)
        if original_tris >= 96:
            target = max(28, min(420, int(original_tris * 0.11)))
            ...
            method = 'quadriflow_preserve_sharp_boundary_per_loose_island'
        else:
            target = original_tris
            method = 'kept_small_source_island'
        part.name = ASSET_ID + '_island_%02d_retopo' % i
        part.data.materials.clear()
        part.data.materials.append(material)
        for p in part.data.polygons:
            p.use_smooth = False
        retopo_parts.append(part)
        island_report.append({
            'island_index': i,
            'original_triangles': int(original_tris),
            'target_faces': int(target),
            'result_vertices': len(part.data.vertices),
            'result_polygons': len(part.data.polygons),
            'result_triangles': source_local_tri_count(part),
            'method': method,
        })
    bpy.ops.object.select_all(action='DESELECT')
    for part in retopo_parts:
        part.select_set(True)
    bpy.context.view_layer.objects.active = retopo_parts[0]
    bpy.ops.object.join()
    obj = bpy.context.object
    obj.name = ASSET_ID + '_upper_front_glacis_island_retopo'
    obj['asset_id'] = ASSET_ID
    obj['scratch_mode'] = True
    obj['revision'] = REVISION
    obj['role'] = ROLE
    ...
    obj['primary_retopo_piece'] = True
    obj['authoring_policy'] = 'Per-loose-island Quadriflow retopo from source component, preserving sharp and boundary features so openings are not bridged. Scratch diagnostic only.'
    obj['island_report_json'] = json.dumps(island_report)
    obj.modifiers.new('weighted_normals_island_retopo', 'WEIGHTED_NORMAL')
    return obj
...
def main():
    ...
    landmarks = {
        'asset_id': ASSET_ID,
        'target_policy': 'per loose source island: max(28, min(420, original_triangles * 0.11))',
        'retopo_engine': 'bpy.ops.object.quadriflow_remesh per loose source island',
        'preserve_sharp': True,
        'preserve_boundary': True,
        'source_component': ROLE,
        'lesson_from_f01_f02_f03': 'f01 had good depth but ugly heightfield artifacts; f02 guessed feature placement; f03 remeshed the whole assembly and bridged openings. f04 separates loose source islands before retopo.'
    }
    ...
    manifest = {
        ...
        'landmarks': {'path': str(landmarks_path.relative_to(ROOT)), 'target_policy': landmarks['target_policy']},
    ...
    note = '# ' + ASSET_ID + '\n\nPer-island source Quadriflow retopo pass. Scratch diagnostic only; not cloud accepted.\n\n- Source component: ' + ROLE + '\n- Stats: ' + str(stats) + '\n- Depth report: ' + str(depth_report) + '\n- Intended to escape f01/f02/f03 ugliness by preserving loose source islands before retopo so openings are not bridged.\n'
