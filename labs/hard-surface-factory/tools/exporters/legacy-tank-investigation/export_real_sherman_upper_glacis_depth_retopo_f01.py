...
ASSET_ID = 'real_sherman_upper_glacis_retopo_landmark_scratch_f01'
REVISION = 'f01-upper-glacis-source-depth-adaptive-reto'
...
def main():
    ...
    hits = sample_grid(source_bvh, cam_basis, GRID_W, GRID_H)
    cells = []
    split_cells(hits, 0, 0, GRID_W, GRID_H, cells)
    retopo_obj, provenance, landmarks = make_retopo_mesh(hits, cells, retopo_mat, source_obj.name)
    ...
    landmarks_path.write_text(json.dumps({'asset_id': ASSET_ID, 'grid': [GRID_W, GRID_H], 'plane_tolerance': PLANE_TOL, 'jump_limit': JUMP_LIMIT, 'cells': landmarks}, indent=2) + '\n')
    provenance_path.write_text(json.dumps({'asset_id': ASSET_ID, 'vertices': provenance}, indent=2) + '\n')
    manifest = {
        'asset_id': ASSET_ID,
        'scratch_mode': True,
        'artifact_type': 'scratch_one_piece_depth_retopo',
        'revision': REVISION,
        'source_glb': str(SOURCE_GLB.relative_to(ROOT)),
        'source_component': ROLE,
        'authoring_policy': 'One-piece adaptive source-depth retopo. Output vertices are ray-supported by the source component and retained for depth or silhouette structure.',
        ...
        'landmarks': {'path': str(landmarks_path.relative_to(ROOT)), 'patch_cells': len(landmarks)},
    ...
    note = '# ' + ASSET_ID + '\n\nOne-piece adaptive source-depth retopo pass.\n\n- Source component: ' + ROLE + '\n- Stats: ' + str(stats) + '\n- Depth report: ' + str(depth_report) + '\n- Vertices are ray-supported by the source component.\n- Scratch diagnostic only; not cloud accepted.\n'
