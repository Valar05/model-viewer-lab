...
ASSET_ID = 'real_sherman_upper_glacis_analytic_primitives_scratch_f06'
REVISION = 'f06-source-measured-analytic-primitive-fused-upper-glacis'
...
def add_projected_uvs(obj):
    mesh = obj.data
    uv_layer = mesh.uv_layers.new(name='uv_textureable_f06')
...
def make_main_shell(classification, zfunc, mat):
    ...
    x0 = q([b['min_x'], b['min_x']], 0) + (b['max_x'] - b['min_x']) * 0.015
    x1 = b['max_x'] - (b['max_x'] - b['min_x']) * 0.010
    y0 = b['min_y'] + (b['max_y'] - b['min_y']) * 0.028
    y1 = b['max_y'] - (b['max_y'] - b['min_y']) * 0.018
    ...
    for i in range(n):
        a = 2.0 * math.pi * i / n
        ox, oy = rect_intersection(cx, cy, a, x0, x1, y0, y1)
    ...
    for i in range(n):
        j = (i + 1) % n
        mb.face([top_outer[i], top_outer[j], top_inner[j], top_inner[i]], 'smooth_plate_deck_frame')
        mb.face([bot_outer[j], bot_outer[i], bot_inner[i], bot_inner[j]], 'underside_closed_shell')
        mb.face([top_outer[j], top_outer[i], bot_outer[i], bot_outer[j]], 'outer_return_wall')
        mb.face([top_inner[i], top_inner[j], bot_inner[j], bot_inner[i]], 'turret_socket_wall')
    obj = mb.make_object()
    obj['surface_class'] = 'smooth_plate_main_armor_shell'
    return obj, {
        'kind': 'closed_radial_plate_shell_with_socket_wall',
        'segments': n,
        'outer_rect': [x0, x1, y0, y1],
...
def make_detail_shells(classification, zfunc, mat):
    ...
    ring = classification['ring']
    cxr, cyr = ring['center']
    for i, cand in enumerate(classification['detail_candidates']):
        sx, sy, sz = cand['size']
        cx, cy, cz = cand['center']
        if sx < 0.012 or sy < 0.012:
            continue
        # Keep only useful readable details, not source scan flakes.
        if used >= 4:
            break
        base_z = zfunc(cx, cy) + 0.006
        height = max(0.012, min(0.030, sz + 0.006))
        ratio = sx / max(0.001, sy)
        if 0.72 <= ratio <= 1.28 and max(sx, sy) < 0.07:
            radius = max(sx, sy) * 0.45
            obj, prim = make_cylinder('round_boss_%02d' % used, cx, cy, radius, base_z, height, mat, 'cylindrical_round_boss')
        else:
            obj, prim = make_rounded_cuboid('raised_detail_%02d' % used, cx, cy, max(sx, 0.026), max(sy, 0.026), base_z, height, mat, 'cuboid_or_rounded_cuboid_hatch_detail')
        details.append(obj)
        prim['source_candidate'] = cand
        primitives.append(prim)
        used += 1
    # Ensure at least one right-deck hatch-like detail exists if noisy components were filtered too hard.
    if not details:
        b = classification['source_bounds']
        x = b['min_x'] + (b['max_x'] - b['min_x']) * 0.77
        y = b['min_y'] + (b['max_y'] - b['min_y']) * 0.72
        base_z = zfunc(x, y) + 0.006
        obj, prim = make_rounded_cuboid('measured_fallback_hatch', x, y, (b['max_x']-b['min_x'])*0.13, (b['max_y']-b['min_y'])*0.09, base_z, 0.020, mat, 'cuboid_or_rounded_cuboid_hatch_detail')
...
def main():
    ...
    note = '# ' + ASSET_ID + '\n\nSource-measured analytic primitive-fused upper glacis scratch pass. Not cloud accepted.\n\n- Stats: ' + str(stats) + '\n- Depth report: ' + str(depth_report) + '\n- Mesh boundary report: ' + str(reports) + '\n'
