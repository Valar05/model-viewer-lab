...
ASSET_ID = 'real_sherman_upper_glacis_f05_revamp_scratch_f11'
REVISION = 'f11-f05-target-low-profile-ring-hatch-cleanup'
...
def measure_features(source_obj):
    ...
    rx_outer = (ring_comp['max'][0] - ring_comp['min'][0]) * 0.5 * 0.94
    ry_outer = (ring_comp['max'][1] - ring_comp['min'][1]) * 0.5 * 0.94
    ring = {
...
def make_f05_revamp(source_obj, armor_mat, ring_mat, hatch_mat):
    ...
    for p in base.data.polygons:
        p.use_smooth = False
        p.material_index = 0
    ring_obj, ring_prim = make_annular_ring(feature, source_bvh, 0)
    ...
    obj['authoring_policy'] = 'f05 visual target preserved; low-profile local measured ring and hatch shells overlay f05 seam zones for texture readiness. No analytic hull restart, no broad deletion.'
...
def main():
    ...
    note = '# ' + ASSET_ID + '\n\nf05 revamp pass: preserve f05 broad hull and overlay low-profile measured local ring/hatch shells. Scratch diagnostic only.\n\n- Stats: ' + str(stats) + '\n- Depth report: ' + str(depth_report) + '\n- Boundary report: ' + str(boundary) + '\n'
